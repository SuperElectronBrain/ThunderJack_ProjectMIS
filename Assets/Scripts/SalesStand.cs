	...
	void Start()
	{
		...
		if (canvas != null)
		{
			if (SalesPanelPrefab != null) 
			{
				...
				FlexibleGridLayout ItemPanelLayout = UniversalFunctions.GetChildOfType<FlexibleGridLayout>(SalesPanel);
				if (ItemPanelLayout != null)
				{
					SalesItemPanel = ItemPanelLayout.gameObject;
				}

				Button button = UniversalFunctions.GetChildOfType<Button>(SalesPanel);
				{
			...
			if (NeedsPanelPrefab != null)
			{
				...
				FlexibleGridLayout ItemPanelLayout = UniversalFunctions.GetChildOfType<FlexibleGridLayout>(NeedsPanel);
				if (ItemPanelLayout != null)
				{
			...
			for (int i = 0; i < selectedItems.Count; i = i + 1)
			{
				if (selectedItems[i].itemAmount > 0)
				{
					if(count < SalesItemPanel.transform.childCount)
					{
						...
						if (image != null)
						{
							text = UniversalFunctions.GetChildOfType<TextMeshProUGUI>(SalesItemPanel.transform.GetChild(count));
		...
		if(customer != null)
		{
			if(NeedsItemPanel != null)
			{
				...
				for (int i = 0; i < (needs.Count < NeedsItemPanel.transform.childCount ? needs.Count : NeedsItemPanel.transform.childCount); i = i + 1)
				{
					...
					if (image != null)
					{
						text = UniversalFunctions.GetChildOfType<TextMeshProUGUI>(NeedsItemPanel.transform.GetChild(i));
